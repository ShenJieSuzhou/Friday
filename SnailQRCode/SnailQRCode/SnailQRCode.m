...
- (void)startScanQRCode:(UIViewController *)rootViewCtrl{
    ...
    if (device) {
        ...
        if (status == AVAuthorizationStatusNotDetermined) {
            ...
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"温馨提示" message:@"请打开相机访问权限开关" preferredStyle:(UIAlertControllerStyleAlert)];
            UIAlertAction *alertA = [UIAlertAction actionWithTitle:@"确定" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
                
            }];
            
            [alertC addAction:alertA];
            [rootViewCtrl presentViewController:alertC animated:YES completion:nil];
