...
/** 扫描内容的Y值 */
#define scanContent_X self.frame.size.width * 0.32
/** 导航条高度*/
...
#pragma mark - - - 扫描外部View的创建
    ...
    CGFloat top_layerY = QRNavbarHeight;
    CGFloat top_layerW = self.frame.size.width;
    CGFloat top_layerH = scanContent_layerY - QRNavbarHeight;
    ...
    CGFloat promptLabelY = CGRectGetMaxY(scanContent_layer.frame) + 15;
...
- (UIImageView *)scanningline {
    if (!_scanningline) {
        ...
        _scanningline.frame = CGRectMake(scanContent_X * 0.8, scanContent_Y, self.frame.size.width - 2 * scanContent_X *0.8, scanninglineHeight);
        
        NSLog(@"%f, %f, %f, %f", _scanningline.frame.origin.x,  _scanningline.frame.origin.y,  _scanningline.frame.size.height,  _scanningline.frame.size.width);
    }
